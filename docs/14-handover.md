...
## 11) تسليح الدوال (2026-09-22) — **مُطبَّق ومُتحقَّق**

سُحب EXECUTE من `PUBLIC` على **كل** دوال المخطط، ثم مُنحت لمستخدم مُصادَق فقط،
و`snapshot_usage` محميَّة بـ`is_admin()` (تسمح بمسار الصيانة بلا هوية).

| الاختبار | قبل | بعد |
|---|---|---|
| دوال قابلة للنداء من `anon` | 35 من 35 | **0** |
| `verify_room_password` بلا تسجيل | 200 | **401** |
| `snapshot_usage` بلا تسجيل | 200 | **401** |
| `is_blocked` بلا تسجيل | 200 | **404** |
| مسار الصيانة (`snapshot_usage` من محرر SQL) | — | ✅ 201 |

ملف الترحيل: `sql/upgrade_2026-09-22_function_hardening.sql`

## 12) لوحة الاستهلاك (نمط Fathom)

تبويب **«الاستهلاك»** في لوحة الإدارة (الويب) يعرض لقطات `usage_snapshots`
الحقيقية: مخطط أعمدة SVG دقيق · أرقام بخط جدولي · رمادي/كحلي + لون تمييز واحد ·
هوامش مصدر لكل رقم (Agora 10,000 دقيقة · Supabase 500 م.ب/1 ج.ب/5 ج.ب/50,000)
وسلوك التخفيض 80/95/100%.
